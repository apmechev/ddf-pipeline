...
def subtract_sources(msfilename,centradeg, centdecdeg, sepradius,sourcelist):
    sourcefile = np.load(sourcelist)

    f = open('find_sources.out','w')

    for source in sourcefile:
        sourcerarad = source[1]
        sourcedecrad = source[2]
        clussep = sepn(sourcerarad,sourcedecrad,clusradeg*deg2rad,clusdecdeg*deg2rad)*rad2deg
        if clussep > sepradius:  ## Anything more than this radius away will be subtracted off
            f.write(str(source) + '\n')
    ...
    sourcefile = open('find_sources.out', 'r')
    for line in sourcefile:
    sourcelist = sourcelist[:-1]
        line = line.split("'")
	sourcelist = sourcelist + line[1] + ','
    sourcelist = sourcelist[:-1]

    runcommand = "killMS.py --MSName %s --SolverType KAFCA --PolMode Scalar --BaseImageName %s --dt %i --Weighting Natural --BeamMode LOFAR --LOFARBeamMode=A --NIterKF 6 --CovQ 0.1 --LambdaKF=%f --NCPU %i --OutSolsName %s --NChanSols %i --InCol %s --kills %s"%(f,imagename,o['dt'], o['LambdaKF'], o['NCPU_killms'], outsols, o['NChanSols'],colname,sourcelist)
	
    #os.system('/net/para11/data1/tasse/killMS_Pack/killMS2/killMS.py --MSName=%s_%s_datafile.ms --SkyModel=%s_sources.bbs.npy --InCol=CORRECTED_DATA --OutCol=DATA --kills=%s --SolverType=CohJones --dt=1 --DoBar=1 --evPStep=10 --NCPU=32'%(clusname,sb,newimagename,sourcelist))
